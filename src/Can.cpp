...
void ClusterFramesSend(void)
{
  ...
  if (F_FILTER_ERR)
    Byte += FUEL_FILTER_IND;
  if (F_GENERAL_WARNING)
    Byte += WARNING_IND;
  if (F_OILPRESS)
    Byte += OILPRESS_IND;
  Data[1] = Byte;
  ...
  if (F_CHECK_ENGINE)
    Byte += CHECKENGINE_IND;
  if (F_CHECK_ENGINE_BLINK)
    Byte += CHECKENGINE_IND_BLINK;
  Data[2] = Byte;
  ...
  if (first_run)
    filtered_fuel_level = fuel; // diasble Exponential moving average for firs reading
  else
    filtered_fuel_level = alfa * fuel + (1 - alfa) * filtered_fuel_level;
  Byte = 0;
  if (filtered_fuel_level <= 8)
    Byte += LOW_FUEL_BLINK; // less than 5 liters or 8%
  else if (filtered_fuel_level <= 16)
    Byte += LOW_FUEL_IND; // less than 10 liters or 16%
